...
public class OrderServiceEx extends OrderService implements IOrderServiceEx
{
	...
	private static final Long BDC_OUT_QUEUE_CALC_NUMBER = 2l;
	
	private static final String BDC_OUT_QUEUE_AUTO_CALC_PROPERTY = "BDC_OUT_QUEUE_AUTO_CALC";
	
	private ISystemServiceEx getSystemServiceEx()
	{
		return ServiceFactory.getService("SystemServiceEx", ISystemServiceEx.class);
	}
	@Override
...
//				String orderNo = subOrderKey[0];
    ...
    public void calcBDCOutQueueForWBS() throws DatasweepException, MESException
	{
		...
		Long notOutCount = bdcOutQueueFilter.getCount();//拿到wbs区域所有未出车的数量
		
		//TODO 2.获取出车队列开始计算数量
		Long calcOutOueueCount = 20l;
		if(notOutCount <= calcOutOueueCount)//如果未出车队列小于或等于设置的出车队列
		{
			...
			MESASOMBDCStockFilter bdcStockFilter = new MESASOMBDCStockFilter();
			bdcStockFilter.forIsinqueueEqualTo(false);//未排队
			bdcStockFilter.forAreaEqualTo(area);
			bdcStockFilter.forIselectroorderEqualTo(true);//电用订单
			bdcStockFilter.addOrderBy(IATRowFilterAttributes.CREATIONTIME, IFilterSortOrders.ASCENDING);//通过创建时间排序
			
			Long electroOrderCount = bdcStockFilter.getCount();//拿到所有的电泳订单数量
			List<IMESASOMBDCStock> electroOrderStockList = bdcStockFilter.getFilteredObjects();//查询所有的结果放入list表中
			...
			bdcStockFilter.addOrderBy(MESGeneratedASOMBDCStock.COL_NAME_BSN, IATRowFilterAttributes.ATCOLUMN, IFilterSortOrders.ASCENDING);//通过创建时间排序
			List<IMESASOMBDCStock> bdcStockList = bdcStockFilter.getFilteredObjects();//把查询所有的BDCk库存中未排队的出车对列
			...
			else if(electroOrderCount>0)//如果电泳件数量大于0
			{
				//BDC库存车辆数量大于19台
			...
			for(IMESASOMBDCRatio bdcRatio:bdcRatioList)
			{
				...
				Long ratioQty = bdcRatio.getRatioqty();//获取占比数量
				Long totalQty = bdcRatio.getTotalqty();//获取总数量
				if(totalQty>0)
				{
					//（车量+电泳件数量）*占比数/总量
					maxQty = (vehicleCalcNum + electroOrderCalcNum)*ratioQty/totalQty;//计算最大的数量
				}
				if(maxQty<1)
				{
					maxQty = 1l;//如果不满一的情况下按i计算
				}
				maxRatioQtyMap.put(bdcRatio.getKey(), maxQty);//最大的数量
				currRatioQtyMap.put(bdcRatio.getKey(), 0l);//当前的数量
			}
			//便利配置项信息
			for(IMESASOMBDCAttrGroup attrGroup : bdcAttrGroupMap.values()) 
			{ 
				...
				{
					Boolean isFind = false;
					String profile = attrGroupItem.getProfile();//配置项名称
					for(String profileName:profileNameList)
					{
						if(profileName.equals(profile))
						{
			...
			for(int i=0;i<vehicleCalcNum;i++)
			{
				...
				{
					prevBdcStock = bdcStockList.get(0);//获取第一条数据
					bdcStockList.remove(prevBdcStock);//把选择的数据从LIst移出
					//把选择的第一条数据放入到ratioMapOfOrder里面
				...
				for(IMESASOMBDCStock bdcStock:bdcStockList)
				{
					...
					if(prevPaintcolor.equals(paintcolor))
					{
						...
						Boolean isExceed = exceedRation(ratioMapOfOrder,maxRatioQtyMap,currRatioQtyMap);//计算是否超出最大数量
						if(isExceed==false)
						{
							//没有超出
							prevBdcStock = bdcStock;
							//拿到的订单从IMESASOMBDCStock列表中移出
			...
			{
				outQueueTempList.add(electroOrderStockList.get(i));
			}
    ...
    private Boolean exceedRation(Map<Long,Boolean> ratioMapOfOrder,Map<Long, Long> maxRatioQtyMap,Map<Long, Long> currRatioQtyMap)
    {
		...
		for (Long key : keys) {
			...
			if(isAccord == true)
			{
				Long maxRatioQty = maxRatioQtyMap.get(keys) - 1l;
   ...
   //向出车对列表中添加出车队列
	public IMESASOMBDCOutQueue addOrderTobdcOutQueue(IMESASOMBDCStock bdcStock, Long groupNo, Long mesSeq) throws MESException
	{
		...
		bdcStock.setIsinqueue(true);//更新队列中AT_OM_BDC_STOCK 的状态为 true
		Response response = bdcStock.save(null, keyWord, null);
		if (response.isOk())
		{
			LogUtility.logInfo(MODEL_NAME, "Save MESASOMBDCStock success,KeyWord<" + keyWord + ">",
		...
		//插入出车都列表中
		Long sortNo = SystemHelperEx.getUserSequenceResponseByName(bdcStock.getArea()+"_OUT_QUEUE");//总装序列号
		IMESASOMBDCOutQueue bdcOutQueue = new MESASOMBDCOutQueue();
		bdcOutQueue.setArea(bdcStock.getArea());
		bdcOutQueue.setBdcseq(0l);//bdc顺序号
		bdcOutQueue.setBsn(bdcStock.getBsn());
		bdcOutQueue.setGroupno(groupNo);//分组号
		bdcOutQueue.setIselectroorder(bdcStock.getIselectroorder());//是否电沪件订单
		bdcOutQueue.setIsmanual(false);//是否强队列
		bdcOutQueue.setIsout(false);//是否拉出
		bdcOutQueue.setMesseq(mesSeq);//mes顺序号
		bdcOutQueue.setOrder(bdcStock.getOrder());
		bdcOutQueue.setOrdernumber(bdcStock.getOrdernumber());
		bdcOutQueue.setPaintcolor(bdcStock.getPaintcolor());//车神颜色
		bdcOutQueue.setRequestcount(0l);//请求代码
		bdcOutQueue.setSendstatus(0l);//发生状态
		bdcOutQueue.setSortno(sortNo);//
